...
namespace Hallo.Util
{
    ...
    { 
        ...
        {
            //Contract.Requires<ArgumentNullException>(text != null && text.Contains(':'));

            string[] splitted = text.Split(':');

            return new IPEndPoint(IPAddress.Parse(splitted[0]), int.Parse(splitted[1]));
        }
        ...
        public static IPAddress ParseIPAddress(string text)
        {
            ...
            IPAddress.TryParse(text, out ip);
        ...
        {
            Check.NotNullOrEmpty(text, "text");
            ...
            {
                int port = 0;
                if (!int.TryParse(splitted[1], out port)) return false;
            }

            return IsIPAddress(splitted[0]);
        }
        ...
        internal static bool IsInteger(string word)
        {
            int integer = 0;
            return int.TryParse(word, out integer); 
        }
    }
}
